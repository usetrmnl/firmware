...
static uint32_t downloadStream(WiFiClient *stream, int content_size,
                               uint8_t *buffer);
...
static void submitOrSaveLogString(const char *log_buffer,
                                  size_t size); // log sending
...
void bl_init(void) {
  ...
  free(framebuffer);
  delay(1000 * 10);
   display_show_msg(storedLogoOrDefault(), WIFI_CONNECT);
   delay(1000 * 10);
#endif
...
uint32_t downloadStream(WiFiClient *stream, int content_size, uint8_t *buffer) {
  ...
  Log_info("Download end: %d/%d bytes in %d ms (%d iterations)", counter,
           content_size, millis() - download_start, iteration_counter);
  return counter;
}
          ...
          if (httpCode > 0) {
            ...
            if (httpCode == HTTP_CODE_OK ||
              ...
              if (stream->available() &&
                ...
                counter =
                    downloadStream(stream, DISPLAY_BMP_IMAGE_SIZE, buffer);
              }
              https.end();
...
int submitLog(const char *format, time_t time, int line, const char *file,
  ...
  LogWithDetails input = {
      ...
      .logRetry = log_retry,
      .retryAttempt =
          log_retry ? preferences.getInt(PREFERENCES_CONNECT_API_RETRY_COUNT)
                    : 0};
